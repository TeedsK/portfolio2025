// src/pages/landing/components/WorkExperience.tsx
...
type ExperienceItem = {
    ...
    role: string;
    team: string;
    description: string[]; // multi-line short summary
    extra?: string[]; // revealed on expand
    tech: Tech[];
};
...
// pick while allowing unknown techs
const colorForTech = (name: string) => TECH_COLORS[name] ?? '#6366F1';

// --- Demo content (adjust freely) ---
const EXPERIENCES: ExperienceItem[] = [
    ...
    },
    {
        ...
        extra: [
            'Partnered with cross‑functional teams to reduce incident MTTR by 38%.',
        ],
        tech: [
            { name: 'Python', color: colorForTech('Python') },
            { name: 'TensorFlow', color: colorForTech('TensorFlow') },
        ...
        <section id="work-experience" className="work-section" aria-labelledby="work-title">
            ...
            <div className="work-cards-viewport">
                <div className="work-cards-rail">
                    ...
                    {/* You can add more <WorkCard/> items above to extend the rail */}
                </div>
            </div>
        </section>
    );
